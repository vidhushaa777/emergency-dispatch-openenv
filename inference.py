...
# ── OpenAI client created LAZILY to avoid crash at module load ──
_client = None
...
# LLM CALL
# ─────────────────────────────────────────────
def get_action(obs: dict, history: list) -> tuple:
...
def run_task(task_name: str) -> dict:
    ...
    while not done:
        step += 1
        action, _ = get_action(obs, history)
        ...
        step_logs.append({
            "step": step, "reward": reward, "done": done,
    ...
    for task_name in TASKS:
        ...
        try:
            ...
            results[task_name] = {
                ...
                "grade_score": 0.0, "steps": 0, "passed": False,
            }
        # ── required [END] output ──
        r = results[task_name]
        print(f"[END] task={task_name} score={r.get('grade_score', 0.0)} steps={r.get('steps', 0)}", flush=True)
